...
/**
 * A {@link View} which is a copy of a {@link View} from the VSUM of a vitruv
 * server.
 * <p>
 * Actions performed on this remote view or to the original view can be
 * synchronized via the network. This view uses a {@link VitruvRemoteConnection}
 * to do so.
 */
public class RemoteView implements View, HasRemoteUuid {
	...
	public ResourceSet getViewSource() {
		return viewSource;
	}

	public void setViewSource(ResourceSet viewSource) {
	 ...
	 * its previous state and now providing an updated view. This can only be done
	 * for an unmodified view.
	 *
	...
	public void moveRoot(EObject object, URI newLocation) {
		...
		viewSource.getResources().stream().filter(it -> it.getContents().contains(object)).findFirst().get()
				.setURI(newLocation);
	}
	 ...
	 * to commit are identified by recording any changes made to the view.
	 *
	 * @throws UnsupportedOperationException if called on a modified view
	 * @throws IllegalStateException         if called on a closed view
	 * @see #isClosed()
	...
	/**
	 * Returns a {@link CommittableView} based on the view's configuration. Changes
	 * to commit are identified by comparing the current view state with its state
